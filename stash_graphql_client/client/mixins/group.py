...
class GroupClientMixin(StashClientProtocol):
    ...
    async def find_group(self, group_id: str) -> Group | None:
        ...
        try:
            ...
            if result and result.get("findGroup"):
                # Sanitize model data before creating Group
                clean_data = sanitize_model_data(result["findGroup"])
                return Group(**clean_data)
            return None
    ...
    async def find_groups(
        ...
        try:
            ...
            return FindGroupsResultType(**result["findGroups"])
        except Exception as e:
    ...
    async def create_group(self, group: Group) -> Group:
        ...
        clean_data = sanitize_model_data(result["groupCreate"])
        return Group(**clean_data)
    ...
    async def update_group(self, group: Group) -> Group:
        ...
        clean_data = sanitize_model_data(result["groupUpdate"])
        return Group(**clean_data)
    ...
    async def group_destroy(
        ...
        result = await self.execute(
            ...
            {"input": input_data.model_dump(exclude_none=True, by_alias=True)},
        )
    ...
    async def bulk_group_update(
        ...
        return [
            Group(**sanitize_model_data(g)) for g in result.get("bulkGroupUpdate", [])
        ]
