...
- **Type guards for three-state (`UnsetType`) fields: `is_present`, `present`, and `is_unset`.** A queried field can be in one of three states — `UNSET` (not queried), `None` (queried, null), or a concrete value — and narrowing all the way to "the value is really here" previously needed a compound `is_set(x) and x is not None` that both mypy and ruff dislike. `is_present(x)` is a `TypeIs` guard (PEP 742) that narrows `T | None | UnsetType` to `T` in a single check (asserting set _and_ non-null); `present(x)` is the value-returning form, returning the concrete value or raising `ValueError` when the field is `UNSET` or `None`. `is_unset(x)` is the dual of `is_set` — it narrows to `UnsetType` in the `if` branch and, because it is a `TypeIs` guard, to the concrete `T` after an early return, enabling a guard-clause style (`if is_unset(scene.path): raise ...` then use `scene.path` as `T` with no wrapper). All three complement the existing `is_set` and are exported from `stash_graphql_client`, `stash_graphql_client.types`, and `stash_graphql_client.types.unset`.

### Changed

- **`is_set` now narrows both branches (`TypeIs` instead of `TypeGuard`).** `is_set` was a `TypeGuard`, which narrowed only the `if` branch; it is now a `TypeIs` guard (PEP 742), so the `else` branch of `if is_set(x)` narrows to `UnsetType`. This is purely additive — the `if`-branch type is unchanged, so existing call sites are unaffected. Implemented via a `TYPE_CHECKING`-only `TypeIs` import, so there is no new runtime dependency and the `>=3.12` floor is unchanged (`TypeIs` is stdlib from 3.13).
