...
class TestPopulateWithFieldsParameter:
    ...
    async def test_populate_with_already_received_fields_no_fetch(
        ...
        # Should not have made another GraphQL call
    ...
    @pytest.mark.asyncio
    @pytest.mark.unit
    async def test_populate_with_empty_fields_list(
        self, respx_stash_client, respx_mock: respx.MockRouter
    ) -> None:
...
class TestFieldMergingOnRefetch:
    ...
    @pytest.mark.asyncio
    @pytest.mark.unit
    async def test_received_fields_merged_on_populate(
        self, respx_stash_client, respx_mock: respx.MockRouter
    ) -> None:
...
class TestForceRefetchParameter:
    ...
    async def test_force_refetch_with_empty_fields_refetches_all(
        ...
        respx.post("http://localhost:9999/graphql").mock(
            return_value=httpx.Response(200, json={"data": initial_data})
        )
    ...
    @pytest.mark.asyncio
    @pytest.mark.unit
    async def test_force_refetch_false_uses_cache(
        self, respx_stash_client, respx_mock: respx.MockRouter
    ) -> None:
        ...
        # Should not make a GraphQL call (field already received)
        assert len(graphql_route.calls) == 0
...
class TestHasFieldsHelper:
    ...
    @pytest.mark.asyncio
    @pytest.mark.unit
    async def test_has_fields_with_no_received_fields(self, respx_stash_client) -> None:
...
class TestPopulateIntegrationWithPhase2:
    ...
    async def test_populate_nested_studio_object_directly(
        ...
        populated_studio = await store.populate(scene.studio, fields=["url", "details"])

        # Verify studio now has additional fields
        assert populated_studio.url == "http://test.com"
    ...
    @pytest.mark.asyncio
    @pytest.mark.unit
    async def test_populate_performer_from_list(
        self, respx_stash_client, respx_mock: respx.MockRouter
    ) -> None:
    ...
    @pytest.mark.asyncio
    @pytest.mark.unit
    async def test_populate_uses_cached_nested_object(
        self, respx_stash_client, respx_mock: respx.MockRouter
    ) -> None:
        ...
        populated_studio = await store.populate(scene1.studio, fields=["url"])
        assert populated_studio.url == "http://shared.com"

        # Scene2's studio should also have the URL (same reference)
        assert scene2.studio is not None
        assert scene2.studio is scene1.studio  # Identity map
        assert scene2.studio.url == "http://shared.com"
