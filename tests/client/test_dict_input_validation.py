...
class TestDictValidationTypeChecking:
    ...
    @pytest.mark.asyncio
    async def test_bulk_image_update_rejects_non_dict(self, respx_stash_client):
        ...
        with pytest.raises(
            ...
            await respx_stash_client.bulk_image_update("invalid")  # type: ignore

    @pytest.mark.asyncio
    async def test_scene_destroy_rejects_non_dict(self, respx_stash_client):
        """scene_destroy should reject non-dict, non-Input inputs."""
        with pytest.raises(
            TypeError, match="input_data must be SceneDestroyInput or dict"
        ):
            await respx_stash_client.scene_destroy(123)  # type: ignore

    @pytest.mark.asyncio
    async def test_tag_destroy_rejects_non_dict(self, respx_stash_client):
        """tag_destroy should reject non-dict, non-Input inputs."""
        with pytest.raises(
            TypeError, match="input_data must be TagDestroyInput or dict"
        ):
            await respx_stash_client.tag_destroy([1, 2, 3])  # type: ignore

    @pytest.mark.asyncio
    async def test_bulk_scene_marker_update_rejects_non_dict(self, respx_stash_client):
        ...
        with pytest.raises(
            ...
            await respx_stash_client.bulk_scene_marker_update(None)  # type: ignore

    @pytest.mark.asyncio
    async def test_performer_destroy_rejects_non_dict(self, respx_stash_client):
        ...
        with pytest.raises(
            ...
            await respx_stash_client.performer_destroy({"id": "123"}.items())  # type: ignore

    @pytest.mark.asyncio
    async def test_bulk_performer_update_rejects_non_dict(self, respx_stash_client):
        ...
        with pytest.raises(
            ...
            await respx_stash_client.bulk_performer_update(None)  # type: ignore

    @pytest.mark.asyncio
    async def test_image_destroy_rejects_non_dict(self, respx_stash_client):
        """image_destroy should reject non-dict, non-Input inputs."""
        with pytest.raises(
            TypeError, match="input_data must be ImageDestroyInput or dict"
        ):
            await respx_stash_client.image_destroy([1, 2, 3])  # type: ignore

    @pytest.mark.asyncio
    async def test_images_destroy_rejects_non_dict(self, respx_stash_client):
        """images_destroy should reject non-dict, non-Input inputs."""
        with pytest.raises(
            TypeError, match="input_data must be ImagesDestroyInput or dict"
        ):
            await respx_stash_client.images_destroy("not valid")  # type: ignore

    @pytest.mark.asyncio
    async def test_scenes_destroy_rejects_non_dict(self, respx_stash_client):
        """scenes_destroy should reject non-dict, non-Input inputs."""
        with pytest.raises(
            TypeError, match="input_data must be ScenesDestroyInput or dict"
        ):
            await respx_stash_client.scenes_destroy(12345)  # type: ignore

    @pytest.mark.asyncio
    async def test_studio_destroy_rejects_non_dict(self, respx_stash_client):
        """studio_destroy should reject non-dict, non-Input inputs."""
        with pytest.raises(
            TypeError, match="input_data must be StudioDestroyInput or dict"
        ):
            await respx_stash_client.studio_destroy({"foo": "bar"}.keys())  # type: ignore
...
class TestDictValidationPydanticValidation:
    ...
    @pytest.mark.asyncio
    async def test_tag_destroy_validates_required_field(self, respx_stash_client):
        """tag_destroy should reject dict missing required 'id' field."""
        with pytest.raises(ValidationError):  # Pydantic ValidationError
            await respx_stash_client.tag_destroy({})  # Missing required 'id'

    def test_image_destroy_validates_required_field(self, respx_stash_client):
        """ImageDestroyInput should validate required 'id' field."""
        with pytest.raises(ValidationError):
            ImageDestroyInput()  # type: ignore  # Missing required 'id'

    def test_images_destroy_validates_required_field(self, respx_stash_client):
...
class TestDictValidationInputTypeAcceptance:
    ...
    def test_scene_destroy_accepts_input_type(self, respx_stash_client):
    ...
    def test_bulk_gallery_update_accepts_input_type(self, respx_stash_client):
    ...
    def test_performer_destroy_accepts_input_type(self, respx_stash_client):
    ...
    def test_bulk_performer_update_accepts_input_type(self, respx_stash_client):
    ...
    def test_image_destroy_accepts_input_type(self, respx_stash_client):
    ...
    def test_images_destroy_accepts_input_type(self, respx_stash_client):
    ...
    def test_studio_destroy_accepts_input_type(self, respx_stash_client):
    ...
    def test_tag_destroy_accepts_input_type(self, respx_stash_client):
    ...
    def test_bulk_scene_marker_update_accepts_input_type(self, respx_stash_client):
    ...
    def test_scene_merge_accepts_input_type(self, respx_stash_client):
...
class TestDictValidationGraphQLConversion:
    ...
    def test_save_filter_converts_to_graphql(self, respx_stash_client):
        """save_filter should convert validated dict to GraphQL format."""
        input_dict: dict[str, Any] = {"mode": FilterMode.SCENES, "name": "Test Filter"}
    ...
    def test_scene_destroy_preserves_optional_fields(self, respx_stash_client):
        """scene_destroy should preserve optional boolean fields in conversion."""
        input_dict: dict[str, Any] = {"id": "123", "delete_file": True}
    ...
    def test_bulk_update_excludes_unset_fields(self, respx_stash_client):
    ...
    def test_images_destroy_converts_to_graphql(self, respx_stash_client):
        """images_destroy should convert dict to GraphQL format."""
        input_dict: dict[str, Any] = {"ids": ["100", "200", "300"], "delete_file": True}
    ...
    def test_bulk_performer_update_converts_to_graphql(self, respx_stash_client):
        """bulk_performer_update should convert dict to GraphQL format."""
        input_dict: dict[str, Any] = {"ids": ["10", "20"], "favorite": True}
...
class TestDictValidationFieldTypes:
    ...
    def test_rating100_validates_range(self, respx_stash_client):
        """BulkImageUpdateInput should validate rating100 is 0-100."""
        # Valid rating
        valid_dict: dict[str, Any] = {"ids": ["1"], "rating100": 85}
    ...
    def test_ids_must_be_list_of_strings(self, respx_stash_client):
        """Bulk update inputs should validate ids is list[str]."""
        # Valid list of strings
        valid_dict: dict[str, Any] = {"ids": ["1", "2", "3"]}
    ...
    def test_boolean_fields_validated(self, respx_stash_client):
        """Input types should validate boolean fields."""
        # Valid boolean
        valid_dict: dict[str, Any] = {"id": "123", "delete_file": True}
...
class TestDictValidationErrorMessages:
    ...
    @pytest.mark.asyncio
    async def test_type_error_includes_actual_type(self, respx_stash_client):
        """TypeError should include the actual type that was passed."""
        with pytest.raises(TypeError) as exc_info:
            await respx_stash_client.tag_destroy(12345)  # type: ignore
    ...
    @pytest.mark.asyncio
    async def test_type_error_includes_method_context(self, respx_stash_client):
        """TypeError should make it clear which parameter is wrong."""
        with pytest.raises(TypeError) as exc_info:
            await respx_stash_client.bulk_performer_update([])  # type: ignore
...
class TestDictValidationCoverage:
    ...
    @pytest.mark.asyncio
    async def test_scene_mixin_coverage(self, respx_stash_client):
        """Scene mixin methods validate dict inputs."""
        # SceneDestroyInput
        with pytest.raises(TypeError):
            await respx_stash_client.scene_destroy("not a dict")  # type: ignore

        # ScenesDestroyInput
        with pytest.raises(TypeError):
            await respx_stash_client.scenes_destroy(123)  # type: ignore

        # SceneMergeInput
        with pytest.raises(TypeError):
            await respx_stash_client.scene_merge([])  # type: ignore

    @pytest.mark.asyncio
    async def test_image_mixin_coverage(self, respx_stash_client):
        """Image mixin methods validate dict inputs."""
        # ImageDestroyInput
        with pytest.raises(TypeError):
            await respx_stash_client.image_destroy("invalid")  # type: ignore

        # ImagesDestroyInput
        with pytest.raises(TypeError):
            await respx_stash_client.images_destroy(None)  # type: ignore

        # BulkImageUpdateInput
        with pytest.raises(TypeError):
            await respx_stash_client.bulk_image_update(42)  # type: ignore

    @pytest.mark.asyncio
    async def test_gallery_mixin_coverage(self, respx_stash_client):
        """Gallery mixin methods validate dict inputs."""
        # BulkGalleryUpdateInput
        with pytest.raises(TypeError):
            await respx_stash_client.bulk_gallery_update("invalid")  # type: ignore

    @pytest.mark.asyncio
    async def test_tag_mixin_coverage(self, respx_stash_client):
        """Tag mixin methods validate dict inputs."""
        # TagDestroyInput
        with pytest.raises(TypeError):
            await respx_stash_client.tag_destroy(None)  # type: ignore

    @pytest.mark.asyncio
    async def test_studio_mixin_coverage(self, respx_stash_client):
        """Studio mixin methods validate dict inputs."""
        # StudioDestroyInput
        with pytest.raises(TypeError):
            await respx_stash_client.studio_destroy([])  # type: ignore

        # BulkStudioUpdateInput
        with pytest.raises(TypeError):
            await respx_stash_client.bulk_studio_update(123)  # type: ignore

    @pytest.mark.asyncio
    async def test_marker_mixin_coverage(self, respx_stash_client):
        """Marker mixin methods validate dict inputs."""
        # BulkSceneMarkerUpdateInput
        with pytest.raises(TypeError):
            await respx_stash_client.bulk_scene_marker_update("not valid")  # type: ignore

    @pytest.mark.asyncio
    async def test_performer_mixin_coverage(self, respx_stash_client):
        """Performer mixin methods validate dict inputs."""
        # PerformerDestroyInput
        with pytest.raises(TypeError):
            await respx_stash_client.performer_destroy([])  # type: ignore
    ...
    @pytest.mark.asyncio
    async def test_filter_mixin_coverage(self, respx_stash_client):
        """Filter mixin methods validate dict inputs."""
        # SaveFilterInput
        with pytest.raises(TypeError):
            await respx_stash_client.save_filter("invalid")  # type: ignore
...
class TestDictValidationGroupMixin:
    ...
    @pytest.mark.asyncio
    async def test_group_destroy_validates_and_constructs(self, respx_stash_client):
        """group_destroy constructs Input from dict and validates."""
        # Valid dict should construct Input successfully
        valid_dict: dict[str, Any] = {"id": "123"}
        input_obj = GroupDestroyInput(**valid_dict)
        assert input_obj.id == "123"

        # Invalid type should raise
        with pytest.raises(TypeError):
            await respx_stash_client.group_destroy(None)  # type: ignore

    @pytest.mark.asyncio
    async def test_bulk_group_update_validates_and_constructs(self, respx_stash_client):
        """bulk_group_update constructs Input from dict and validates."""
        # Valid dict
        valid_dict: dict[str, Any] = {"ids": ["1", "2"], "rating100": 90}
        input_obj = BulkGroupUpdateInput(**valid_dict)
        assert input_obj.ids == ["1", "2"]

        # Invalid type should raise
        with pytest.raises(TypeError):
            await respx_stash_client.bulk_group_update("not a dict")  # type: ignore
